...
# %% colab={"base_uri": "https://localhost:8080/"} id="3SamlGZPoDtz" outputId="1c3df2a9-7b01-449a-9a16-416f306a10ab"
from google.colab import drive
drive.mount('/content/drive')

# %% colab={"base_uri": "https://localhost:8080/"} id="slkCcr5PoGN5" outputId="115dd945-f2b9-400c-ece7-5543ba83de34"
import os
import zipfile

# Define the target directory in Google Drive for the unzipped files
# This path is derived from the np.loadtxt calls in a subsequent cell (RmH5_HJyoLmZ)
...
# This is a more robust check to confirm data readiness in Google Drive
...
# %% id="RmH5_HJyoLmZ" colab={"base_uri": "https://localhost:8080/"} outputId="1d4ac184-18bd-407b-f94f-84e773dc590c"
...
# Make sure UNZIPPED_DATA_DIR is defined. If this cell is run independently,
# uncomment and define it here or ensure the cell defining it runs first.
# UNZIPPED_DATA_DIR = '/content/drive/MyDrive/data/'
...
# %% colab={"base_uri": "https://localhost:8080/", "height": 1000} id="bjlXsCZioNUj" outputId="f59ad376-894a-461b-b716-573c464a9da1"
...
# %% colab={"base_uri": "https://localhost:8080/"} id="trnQ7AZIoWAh" outputId="b7d667ab-88aa-4b6c-951f-0cf4bdfba76d"
...
# %% colab={"base_uri": "https://localhost:8080/"} id="xiE0jqnvoYw5" outputId="82eee2a4-cb64-47f9-ad9a-e63d43cac3e9"
print(accuracy_score(np.argmax(testY_CNN, axis=1), np.argmax(model.predict(testX_CNN), axis=1)))

# %% colab={"base_uri": "https://localhost:8080/"} id="YRtFdK88ocYR" outputId="b1462055-0f46-4806-b681-89fe00b0a69e"
print(classification_report(np.argmax(testY_CNN, axis=1), np.argmax(model.predict(testX_CNN), axis=1)))

# %% colab={"base_uri": "https://localhost:8080/", "height": 489} id="4G5zcYrUoeAI" outputId="8369e59f-fe82-48ec-9434-8aec6d3f527e"
from sklearn.metrics import precision_recall_curve, auc

# Get predicted probabilities for the positive class
y_pred_proba = model.predict(testX_CNN)[:, 1]
...
# %% colab={"base_uri": "https://localhost:8080/"} id="E5fDzIMRofjU" outputId="452bff54-52de-4948-8b24-d91ab23961db"
print(accuracy_score(np.argmax(trainY_CNN, axis=1), np.argmax(model.predict(trainX_CNN), axis=1)))

# %% colab={"base_uri": "https://localhost:8080/"} id="lezk-933ont4" outputId="5a6eb728-6243-4c13-a41c-e52c17ff6762"
print(classification_report(np.argmax(trainY_CNN, axis=1), np.argmax(model.predict(trainX_CNN), axis=1)))

# %% colab={"base_uri": "https://localhost:8080/", "height": 489} id="SrH6uFDKo4a_" outputId="3efc1d97-604c-49bd-93e5-377aec24aaa0"
from sklearn.metrics import precision_recall_curve, auc

# Get predicted probabilities for the positive class
y_pred_proba = model.predict(trainX_CNN)[:, 1]
...
# %% colab={"base_uri": "https://localhost:8080/"} id="h69nezDzo91A" outputId="e1a269a1-e73b-437c-e7af-ec6f799c37b7"
""" TRADING STRATEGY BEFORE ATTACK ON TEST DATA"""
import numpy as np
import pandas as pd

print("Getting predictions...")
train_predictions = model.predict(testX_CNN)
...
# %% colab={"base_uri": "https://localhost:8080/"} id="2qc3yLhUpFW3" outputId="ba97051b-5c5f-4b54-e5bb-8dc83e501568"
...
# %% colab={"base_uri": "https://localhost:8080/"} id="ez4jB4EFpc9f" outputId="30cd9817-ceef-4146-a1fc-23a428ec9879"
...
def pgd_attack(images, labels, epsilon):
    ...
    for _ in range(num_iterations):
        ...
        # L∞ projection + volume floor: delta ∈ [0, epsilon]
...
def random_noise_attack(images, epsilon):
    """Random noise baseline attack: uniform noise, L∞ + volume floor."""
...
# %% colab={"base_uri": "https://localhost:8080/"} id="YXVKEoISpq1g" outputId="73fefec1-892a-43a6-df08-dbd1d89c3476"
""" ADVERSARIAL ATTACK ON TEST DATA ON 4 EPSILON VALUES"""
import numpy as np
import tensorflow as tf
from sklearn.metrics import accuracy_score
...
# %% colab={"base_uri": "https://localhost:8080/"} id="Qtz8ApMOsA6G" outputId="58488667-10ec-484e-8491-eea3726a7328"
...
# %% colab={"base_uri": "https://localhost:8080/"} id="FL4fL6U0uRFE" outputId="81578dab-2032-4529-df8d-817449dea8bf"
"""TRADING STRATEGY AFTER ATTACK ON 4 EPSILON VALUES"""
import numpy as np
import pandas as pd
import tensorflow as tf
